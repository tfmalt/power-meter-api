...
ctrl.read.connect().then(function () {
    logger.info("Connected to redis: ", config.read);
}, function (err) {
    logger.error("Failed connecting to redis: " + err, config.read);
});
ctrl.write.connect().then(function () {
    logger.info("Connected to redis: ", config.write);
}, function (err) {
    logger.error("Failed connecting to redis: " + err, config.write);
});
